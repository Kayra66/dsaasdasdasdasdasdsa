...
const words = ['çikolata', 'aggsy', 'odun', 'bomba', 'süt', 'şimşek', 'yıldırım', 'patlat', 'savaş', 'kelime', 'yok et', 'bilmem', 'öldür', 'sonsuzluk', 'gerçek', "egehanss"];
...
exports.run = async (client, msg, args) => {
  ...
  if (opponent.bot) return msg.reply('Botlar ile oynayamazsın!');
		...
		try {
			await msg.channel.send(`<a:ates:739608060942614559> ${opponent} <a:ates:739608060942614559> bu meydan okumayı kabul ediyor musun? (\`evet\` veya \`hayır\` olarak cevap veriniz.) <a:ates:739608060942614559> `);
			...
			return msg.channel.send(`Hızlıymışsın! Tebrikler ${winner.first().author} Kazandın! <a:ates:739608060942614559> `);
		} catch (err) {
			this.fighting.delete(msg.channel.id);
			throw err;
		}
